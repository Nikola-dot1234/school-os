...
function eventOccursOn(event,dateString){
  const start=dateObj(event.date);
  const current=dateObj(dateString);
  const end=dateObj(event.endDate||event.date);
  ...
  let interval=1;
  if(event.repeat==="weekly")interval=7;
  if(event.repeat==="biweekly")interval=14;
  ...
  if(event.repeat==="monthly"){
    ...
    occurrenceStart.setMonth(start.getMonth()+monthDiff);
  ...
  if(diff%interval!==0){
    // For multi-day recurring events, also show the days inside the occurrence.
    const occurrenceStartDays=diff-(diff%interval);
    return diff>=occurrenceStartDays&&diff<=occurrenceStartDays+durationDays;
  }
...
function eventDateLabel(event){
  ...
  return endDate!==event.date?`${start} – ${formatDate(endDate)}`:start;
}
function eventRepeatLabel(repeat){
  return ({daily:"Every day",weekly:"Every week",biweekly:"Every 2 weeks",monthly:"Every month"}[repeat])||"Does not repeat";
}
...
function calendarItemsForDate(dateString){
  const items=[];
  eventsOn(dateString).forEach(e=>items.push({type:"event",time:e.start||"",text:e.name,id:e.id,event:e}));
  tasks.filter(t=>t.due===dateString).forEach(t=>items.push({type:"task",time:"",text:`✓ ${t.name}`}));
  tests.filter(t=>t.date===dateString).forEach(t=>items.push({type:"test",time:"",text:`🧪 ${t.subject}`}));
  return items;
}
function eventMarkup(item){
  const e=item.event;
  return `<button type="button" class="calendar-event calendar-personal event-clickable" onclick="openEventDetails('${e.id}')"><span>${escapeHTML(item.text)}</span>${item.time?`<small>${item.time}${e.end?`–${e.end}`:""}</small>`:""}</button>`;
}
...
function monthCalendarHTML(){
  const y=calendarDate.getFullYear(),m=calendarDate.getMonth(),first=new Date(y,m,1);
  let start=first.getDay();if(start===0)start=7;
  const total=new Date(y,m+1,0).getDate();let cells="";
  for(let i=1;i<start;i++)cells+=`<div class="calendar-day empty-day"></div>`;
  for(let d=1;d<=total;d++){
    const ds=calendarDateString(y,m,d),today=ds===todayString();
    cells+=`<div class="calendar-day ${today?"today":""}"><div class="calendar-day-number">${d}</div>${calendarItemsForDate(ds).map(x=>x.type==="event"?eventMarkup(x):`<div class="calendar-event calendar-${x.type}">${escapeHTML(x.text)}${x.time?` · ${x.time}`:""}</div>`).join("")}</div>`;
  }
  return `<div class="calendar-weekdays"><div>Mon</div><div>Tue</div><div>Wed</div><div>Thu</div><div>Fri</div><div>Sat</div><div>Sun</div></div><div class="calendar-grid">${cells}</div>`;
}
...
function weekCalendarHTML(){
  ...
  for(let d=0;d<7;d++){
    ...
    const school=timetable[dayNumber]||[];
    const dayEvents=eventsOn(ds).sort((a,b)=>(a.start||"").localeCompare(b.start||""));
    const dayTasks=tasks.filter(x=>x.due===ds);
    const dayTests=tests.filter(x=>x.date===ds);
    const schedule=school.map((subject,i)=>{const t=times[i+1];return `<div class="week-class"><span class="week-class-time">${t[1]}</span><span class="week-class-subject ${subject==="Free"?"free":""}">${escapeHTML(subject)}</span></div>`}).join("");
    const eventHTML=dayEvents.map(e=>`<button type="button" class="week-event" onclick="openEventDetails('${e.id}')"><span>${escapeHTML(e.name)}</span><small>${e.start||""}${e.end?`–${e.end}`:""}</small></button>`).join("");
    const taskHTML=dayTasks.map(t=>`<div class="week-event task"><span>✓ ${escapeHTML(t.name)}</span></div>`).join("");
    const testHTML=dayTests.map(t=>`<div class="week-event test"><span>🧪 ${escapeHTML(t.subject)}</span></div>`).join("");
    dayCards.push(`<section class="week-day-card ${ds===todayString()?"today":""}"><div class="week-day-header"><div><div class="week-day-name">${date.toLocaleDateString("en-GB",{weekday:"long"})}</div><div class="week-day-date">${date.toLocaleDateString("en-GB",{day:"numeric",month:"short"})}</div></div>${ds===todayString()?`<span class="week-today-pill">Today</span>`:""}</div>${eventHTML?`<div class="week-section"><div class="week-section-label">Events</div>${eventHTML}</div>`:""}${taskHTML||testHTML?`<div class="week-section"><div class="week-section-label">School OS</div>${taskHTML}${testHTML}</div>`:""}<div class="week-section"><div class="week-section-label">Timetable</div>${schedule}</div></section>`);
  }
  return `<div class="week-agenda">${dayCards.join("")}</div>`;
}
...
function openEventModal(){
  ...
  document.getElementById("event-end-date").value=todayString();
...
function openEventDetails(id){
  ...
  if(!event)return;
  document.getElementById("event-details-name").textContent=event.name;
  document.getElementById("event-details-date").textContent=eventDateLabel(event);
  document.getElementById("event-details-time").textContent=event.start&&event.end?`${event.start}–${event.end}`:(event.start||"All day");
  document.getElementById("event-details-repeat").textContent=eventRepeatLabel(event.repeat);
  document.getElementById("event-details-delete").onclick=()=>deleteEvent(id);
  document.getElementById("event-details-modal").classList.remove("hidden");
}
function closeEventDetails(){document.getElementById("event-details-modal").classList.add("hidden")}
...
document.getElementById("close-event-modal").addEventListener("click",closeEventModal);document.getElementById("cancel-event").addEventListener("click",closeEventModal);document.getElementById("event-date").addEventListener("change",()=>{const start=document.getElementById("event-date").value;const end=document.getElementById("event-end-date");if(start&&(!end.value||end.value<start))end.value=start;end.min=start});document.getElementById("event-form").addEventListener("submit",e=>{e.preventDefault();const start=document.getElementById("event-date").value;const end=document.getElementById("event-end-date").value||start;if(end<start)return;events.push({id:Date.now().toString(),name:document.getElementById("event-name").value.trim(),date:start,endDate:end,start:document.getElementById("event-start").value,end:document.getElementById("event-end").value,repeat:document.getElementById("event-repeat").value});saveData();closeEventModal();showPage("calendar")});document.getElementById("close-event-details").addEventListener("click",closeEventDetails);document.getElementById("cancel-event-details").addEventListener("click",closeEventDetails);
